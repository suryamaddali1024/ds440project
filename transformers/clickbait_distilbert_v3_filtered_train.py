...
Fine-tune DistilBERT for clickbait detection with three improvements:
  ...
  3. Filter ambiguous training samples: only train on clear-cut examples
     where truthMean < 0.3 (clearly not clickbait) or > 0.7 (clearly clickbait)

Same test split (80/20, stratified on truthClass, seed=42) for fair comparison.
Val set is also unfiltered to match real-world evaluation conditions.

Sections:
  1. Data Loading & Preprocessing (with ambiguous sample filtering)
  2. Model Setup (DistilBERT + regression head)
  3. Training (MSE loss on truthMean, clean training data only)
...
OUTPUT_FILE = "../results/clickbait_predictions_transformer_v3.csv"
...
VAL_SIZE = 0.10
RANDOM_STATE = 42
AMBIG_LOW = 0.3        # truthMean below this = clearly not clickbait
AMBIG_HIGH = 0.7       # truthMean above this = clearly clickbait
...
def load_and_split():
    ...
    # Filter ambiguous samples from TRAINING only
    # Keep val/test unfiltered for fair real-world evaluation
    train_truth_mean = scores[train_idx]
    clear_mask = (train_truth_mean < AMBIG_LOW) | (train_truth_mean > AMBIG_HIGH)
    train_idx_filtered = train_idx[clear_mask]

    n_before = len(train_idx)
    n_after = len(train_idx_filtered)
    n_removed = n_before - n_after
    print(f"\n   Filtering ambiguous training samples (truthMean between {AMBIG_LOW} and {AMBIG_HIGH}):")
    print(f"   Before: {n_before}  After: {n_after}  Removed: {n_removed} ({100*n_removed/n_before:.1f}%)")
    print(f"   Filtered train truthMean distribution:")
    filtered_scores = scores[train_idx_filtered]
    print(f"     Clear non-clickbait (< {AMBIG_LOW}): {(filtered_scores < AMBIG_LOW).sum()}")
    print(f"     Clear clickbait (> {AMBIG_HIGH}): {(filtered_scores > AMBIG_HIGH).sum()}")

    def gather(lst, idxs):
        return [lst[i] for i in idxs]

    train_posts, val_posts, test_posts = gather(post_texts, train_idx_filtered), gather(post_texts, val_idx), gather(post_texts, test_idx)
    train_titles, val_titles, test_titles = gather(title_texts, train_idx_filtered), gather(title_texts, val_idx), gather(title_texts, test_idx)
    train_scores, val_scores, test_scores = scores[train_idx_filtered], scores[val_idx], scores[test_idx]
    test_binary = binary_labels[test_idx]

    print(f"   Train: {len(train_idx_filtered)} (filtered)  Val: {len(val_idx)}  Test: {len(test_idx)}")
...
def print_comparison(f1, acc, prec, rec):
    ...
    print(f"   {'-'*35}  {'-'*7}  {'-'*7}  {'-'*7}  {'-'*7}")
    print(f"   {'DistilBERT v3 (filtered train)':35s}  {f1:7.4f}  {acc:7.4f}  {prec:7.4f}  {rec:7.4f}")

    best_prior = 0.7082  # v2 DistilBERT
    diff = f1 - best_prior
    if diff > 0:
        print(f"\n   >>> v3 beats v2 by +{diff:.4f} F1")
    else:
        print(f"\n   >>> v3 is {diff:+.4f} F1 vs v2")
