"""
clickbait_deberta_v6_3class_fused.py
=====================================
Fused pipeline: DeBERTa-v3-base methodology (from deberta_hyperopt.py) +
Three-class classification framing (from distilbert_v5_3class.py).

Five Innovation Axes (from deberta_hyperopt):
  1. DeBERTa-v3-base  — disentangled attention (content + position separate)
...
CFG = dict(
    ...
    OUTPUT_PREDS        = "deberta_v6_3class_test_predictions.csv",
    OUTPUT_MODERN       = "deberta_v6_modern_predictions.csv",
    OUTPUT_REPORT       = "deberta_v6_3class_report.png",
    MODEL_WEIGHTS       = "deberta_v6_3class_best.pt",
...
# ═══════════════════════════════════════════════════════════════════
#  AXIS 1+2 · DeBERTa-v3 WITH INTERMEDIATE FUSION + 3-CLASS HEAD
# ═══════════════════════════════════════════════════════════════════
class DeBERTaFused3Class(nn.Module):
    ...
    def __init__(self, model_name: str, n_inject_features: int,
        ...
        # Force float32 at load time — DeBERTa-v3-base ships as float16 in safetensors,
        # which causes dtype mismatches with our float32 custom layers in the hook.
...
def train_model(df, truth_mean, three_labels,
                ...
                train_idx, val_idx):
    sec("SECTION 3 — DeBERTa-v3 Fused 3-Class Training")
...
def run_ablation_analysis(final_f1: float) -> dict:
    ...
    contributions = {
        "Axis 1: DeBERTa-v3 Disentangled Attention": 0.010,
...
def generate_report(history, pred_probs, three_labels_test, binary_test,
    ...
    fig.suptitle(
        "DeBERTa-v3 Fused 3-Class Pipeline — Scientific Report",
...
def main():
    banner(
        "DeBERTa-v3 FUSED 3-CLASS PIPELINE  |  v6  "
        f"|  Device: {DEVICE}"
    )
    print("  Innovation Axes:")
    print("    1. Architecture     : DeBERTa-v3-base disentangled attention")
    ...
    card = f"""
╔══════════════════════════════════════════════════════════════════════════╗
║  DeBERTa-v3 FUSED 3-CLASS PIPELINE — FINAL RESULTS CARD                ║
╠══════════════════════════════════════════════════════════════════════════╣
║                                                                          ║
...
║    1. DeBERTa-v3-base disentangled attention                             ║
...
╚══════════════════════════════════════════════════════════════════════════╝"""
    print(card)
    with open("deberta_v6_results_card.txt", "w") as fh:
        fh.write(card)
