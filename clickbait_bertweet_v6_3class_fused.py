"""
clickbait_bertweet_v6_3class_fused.py
======================================
Same fused 3-class pipeline — backbone swapped to vinai/bertweet-base.
BERTweet was pre-trained on 850 M English tweets, giving it native command
of the informal register (slang, abbreviations, emoji-adjacent phrasing)
that characterises clickbait social-media posts.

Five Innovation Axes (from deberta_hyperopt):
  1. BERTweet  — 850M tweet pre-training (vinai/bertweet-base)
...
BERTweet tokenizer is loaded with normalization=True, which maps URLs →
"HTTPURL" and @mentions → "@USER" — the same normalisation used during
pre-training, which improves token coverage on real tweets.
...
CFG = dict(
    ...
    OUTPUT_PREDS        = "bertweet_v6_3class_test_predictions.csv",
    OUTPUT_MODERN       = "bertweet_v6_modern_predictions.csv",
    OUTPUT_REPORT       = "bertweet_v6_3class_report.png",
    MODEL_WEIGHTS       = "bertweet_v6_3class_best.pt",
...
# ═══════════════════════════════════════════════════════════════════
#  AXIS 1+2 · BERTWEET WITH INTERMEDIATE FUSION + 3-CLASS HEAD
# ═══════════════════════════════════════════════════════════════════
class DeBERTaFused3Class(nn.Module):
    ...
    def __init__(self, model_name: str, n_inject_features: int,
        ...
        # BERTweet loads in float32 by default; torch_dtype kept explicit for safety.
...
def train_model(df, truth_mean, three_labels,
                ...
                train_idx, val_idx):
    sec("SECTION 3 — BERTweet Fused 3-Class Training")
...
def run_ablation_analysis(final_f1: float) -> dict:
    ...
    contributions = {
        "Axis 1: BERTweet Tweet-Domain Pre-Training": 0.010,
...
def generate_report(history, pred_probs, three_labels_test, binary_test,
    ...
    fig.suptitle(
        "BERTweet Fused 3-Class Pipeline — Scientific Report",
...
def main():
    banner(
        "BERTweet FUSED 3-CLASS PIPELINE  |  v6  "
        f"|  Device: {DEVICE}"
    )
    print("  Innovation Axes:")
    print("    1. Architecture     : BERTweet (850M tweet pre-training)")
    ...
    card = f"""
╔══════════════════════════════════════════════════════════════════════════╗
║  BERTweet FUSED 3-CLASS PIPELINE — FINAL RESULTS CARD               ║
╠══════════════════════════════════════════════════════════════════════════╣
║                                                                          ║
...
║    1. BERTweet tweet-domain pre-training (850M tweets)               ║
...
╚══════════════════════════════════════════════════════════════════════════╝"""
    print(card)
    with open("bertweet_v6_results_card.txt", "w") as fh:
        fh.write(card)
